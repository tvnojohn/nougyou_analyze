...
#outfile <- args[2]
outglaph <- paste(args[2],"pdf", sep = ".")#2番目の引数を出力ファイルとして>    代入する
...
sink(file = outtxt)
head(d)
#pdf("Wind.Direction.pdf")
pdf(outglaph) #出力ファイルであるPDFの準備をする
ansmeanvar=cpt.meanvar(d[,ncol(d)], method='PELT')
plot(ansmeanvar,cpt.width=3)
ansmean=cpt.mean(d[,ncol(d)], method='PELT')
plot(ansmean, cpt.col='blue')
ansvar=cpt.var(d[,ncol(d)], method='PELT')
plot(ansvar)
print(ansmeanvar)
print(ansvar)
print(ansmean)
